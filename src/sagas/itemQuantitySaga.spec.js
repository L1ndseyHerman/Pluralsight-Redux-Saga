//  Only testing one function from the itemQuantitySaga.
//  fetch
...
describe("item quantity saga", () => {
  ...
  describe("handle increase item quantity", () => {
    ...
    beforeEach(() => {
      ...
      //    User in next one bec need for next yields or something:
    ...
    test("increasing quantity successfully", () => {
      expect(
        gen
          .next({ status: 200 })
          .value.toEqual(put(setItemQuantityFetchStatus(FETCHED)))
      );
    });
    ...
    test("increasing quantity unsuccessfully", () => {
      expect(
        gen
          .next({ status: 500 })
          .value.toEqual(put(decreaseItemQuantity(item.id, true)))
      );
      expect(
        gen.next().value.toEqual(put(setItemQuantityFetchStatus(FETCHED)))
      );
    });
  });
});
